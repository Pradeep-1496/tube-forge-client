                ...
                <tr>
                  ...
                  <td class="col-actions">
                    <div class="row-actions">
                      <button class="btn row-btn" (click)="viewDraft(draft)" title="View">
                        👁
                      </button>
                      <a
                        class="btn row-btn edit"
                        [routerLink]="['/draft/edit', draft.id]"
                        title="Edit"
                      >
                        ✎
                      </a>
                      <button
                        class="btn row-btn generate"
                        (click)="generateFromDraft(draft)"
      ...
      <div class="modal-overlay" (click)="closeView()">
        <div class="modal" (click)="$event.stopPropagation()">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item">
                ...
                <span class="detail-value">{{
                  draft.subscribeImage?.name || draft.subscribeImageId || '—'
                }}</span>
              </div>
              <div class="detail-item">
          ...
          <div class="modal-actions">
            ...
            <button
              class="btn primary"
              (click)="closeView(); generateFromDraft(draft)"
              [disabled]="generatingId() === draft.id"
            >
  ...
  private loadDrafts() {
    ...
    this.api.getDraftVideos().subscribe({
      ...
      error: (err) => {
        ...
        if (status === 403) this.error.set("You don't have permission to view drafts");
        else this.error.set(err?.error?.message || err?.message || 'Failed to load drafts');
        this.loading.set(false);
      },
    });
  }
