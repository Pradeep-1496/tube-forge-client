...
import { ApiService, VideoContent, BackgroundVideo, GenerateFromVideoResponse } from '../../services/api.service';
import { AppSelectComponent, SelectOption } from '../../components/shared/app-select/app-select.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, AppSelectComponent, GenerationProgressComponent, VideoResultComponent],
  template: `
    <div class="page">
      ...
      @if (view() === 'result') {
        ...
        <div class="result-section" *ngIf="responseData() as res">
          <div class="result-meta">
            ...
            <div class="meta-grid">
              <div class="meta-item"><span class="label">Title</span><span class="value">{{ res.metadata.title }}</span></div>
              <div class="meta-item"><span class="label">Status</span><span class="value">{{ res.metadata.status }}</span></div>
              <div class="meta-item"><span class="label">Visibility</span><span class="value">{{ res.metadata.visibility }}</span></div>
              <div class="meta-item"><span class="label">Privacy</span><span class="value">{{ res.metadata.privacy_status }}</span></div>
              <div class="meta-item"><span class="label">Language</span><span class="value">{{ res.metadata.default_language }}</span></div>
              <div class="meta-item"><span class="label">Made for Kids</span><span class="value">{{ res.metadata.self_declared_made_for_kids ? 'Yes' : 'No' }}</span></div>
              <div class="meta-item"><span class="label">Created</span><span class="value">{{ res.metadata.createdAt | date:'medium' }}</span></div>
  ...
  styles: [`
    ...
    .result-meta { background: var(--border-subtle); border: 1px solid var(--border); border-radius: 0.9rem; padding: 1.2rem 1.4rem; }
    .result-meta h3 { margin: 0 0 0.8rem; color: var(--text); font-size: 0.95rem; }
    .meta-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.6rem; }
    .meta-item { display: flex; flex-direction: column; gap: 0.15rem; }
    .meta-item.tags-item { grid-column: 1 / -1; }
    ...
    .tag { font-size: 0.75rem; padding: 0.15rem 0.5rem; background: var(--border); border-radius: 9999px; color: var(--text); }
...
export class GenerateFromVideoComponent implements OnInit {
  ...
  private loadData() {
    ...
    this.api.getBackgroundVideos().subscribe({
      next: (items) => {
        this.bgVideoOptions.set(items.map(i => ({ value: i.id, label: `${i.name} (${i.type})` })));
      },
    });
    ...
    this.api.getChannels().subscribe({
      next: (items) => {
        this.channelOptions.set(items.map(i => ({ value: i.id, label: i.name })));
      },
    });
  }
  ...
  onGenerate() {
    ...
    this.api.generateFromVideo(this.selectedContentId, this.selectedBgVideoId, {
