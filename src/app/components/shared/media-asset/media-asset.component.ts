...
@Component({
  ...
  template: `
    <div class="media-wrap" [class.has-label]="!!label()" [class.aspect-9-16]="aspect() === '9/16'" [class.aspect-16-9]="aspect() === '16/9'" [class.aspect-4-3]="aspect() === '4/3'">
      @if (loading()) {
        <div class="skeleton" [class.skeleton-round]="type() === 'audio'">
  ...
  styles: [`
    ...
    img.hidden, video.hidden, audio.hidden { display: none; }
    audio { width: 100%; }
    audio::-webkit-media-controls-panel { background: var(--border-subtle); }
    .skeleton { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; }
    ...
    .fallback { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.3rem; padding: 1rem; color: var(--muted); }
    .fallback-icon { font-size: 1.5rem; }
    .fallback-text { font-size: 0.78rem; text-align: center; }
    .media-label { font-size: 0.78rem; color: var(--muted); text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }
  `]
})
